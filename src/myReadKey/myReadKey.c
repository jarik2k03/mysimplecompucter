...
int rk_readkey(enum keys* k)
{
    ...
    if (*buffer == 'l') *k = load;
    else if (*buffer == 's') *k = save;
    else if (*buffer == 'r') *k = run;
    else if (*buffer == 't') *k = step;
    else if (*buffer == 'i') *k = reset;
    else if (*buffer == '\n') *k = enter;
    else if (strncmp(buffer, F5_KEY, 4) == 0) *k = f5;
    else if (strncmp(buffer, F6_KEY, 4) == 0) *k = f6;
    else if (strncmp(buffer, DOWN_KEY, 3) == 0) *k = down;
...
int rk_mytermregime(int8_t regime, int8_t vtime, int8_t vmin, int8_t echo, int8_t sigint)
{
    ...
    if (vtime != 0 && vtime != 1) return -1;
    if (vmin != 0 && vmin != 1) return -1;
    if (echo != 0 && echo != 1) return -1;
    if (sigint != 0 && sigint != 1) return -1;
    struct termios termsets;
    tcgetattr(0, &termsets);

    if (regime == 0) termsets.c_lflag &= ~ICANON; // настраиваем неканонический режим.
    else termsets.c_lflag |= ICANON; // настраиваем канонический режим.

    if (vtime == 0) termsets.c_lflag &= ~VTIME;
    else termsets.c_lflag |= VTIME;

    if (vmin == 0) termsets.c_lflag &= ~VMIN;
    else termsets.c_lflag |= VMIN;

    if (echo == 0) termsets.c_lflag &= ~ECHO; // отвечает за невидимость введенной клавиши
    else termsets.c_lflag |= ECHO;

    if (sigint == 0) termsets.c_lflag &= ~ISIG; // позволяет принудительно останавливать и убивать программу
    else termsets.c_lflag |= ISIG;
    tcsetattr(0, TCSANOW, &termsets);
