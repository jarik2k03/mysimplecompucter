...
int bc_printUB(int len) {
  if (bc_printA(LU) == -1) return -1;

  for (int i = 0; i != len; ++i)
    if (bc_printA(HR) == -1) return -1;

  if (bc_printA(RU) == -1) return -1;
...
int bc_printLB(int len) {
  if (bc_printA(LB) == -1) return -1;

  for (int i = 0; i != len; ++i)
    if (bc_printA(HR) == -1) return -1;

  if (bc_printA(RB) == -1) return -1;
...
int bc_box(int x1, int y1, int x2, int y2) {
  ...
  if (mt_gotoXY(x1, y1) == -1) return -1;

  // чердак.
  if (bc_printUB(y2) == -1) return -1;
  ++x1;

  for (int i = 0; i != x2; ++i) {
    mt_gotoXY(x1++, y1);

    if (bc_printA(VT_BOUND) == -1) return -1;
  ...
  // подвал.
  if (bc_printLB(y2) == -1) return -1;

  return 0;
}

int bc_printbigchar(int arr[2], int x, int y, int front, int back) {
  int a[2];
  a[0] = arr[0];
  a[1] = arr[1];
