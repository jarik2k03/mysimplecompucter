...
static void _bstree_delete3(bstree *deltree, char *key) {
  ...
  /* no children*/
  if (!reserve->left && !reserve->right) _bstree_delete1(reserve, key);
...
bstree *bstree_delete(bstree *tree, char *key) {
  ...
  if (!deltree) return tree;

  /* no children*/
  if (!deltree->left && !deltree->right) _bstree_delete1(deltree, key);
  ...
  /* two children */
  if (deltree->left && deltree->right) _bstree_delete3(deltree, key);

  return tree;
}

void bstree_free(bstree *tree) {
  if (tree->left) bstree_free(tree->left);
  if (tree->right) bstree_free(tree->right);
  free(tree);
}

void print_tree_as_list(bstree *root) {
  if (!root) return;
  if (root) printf("[%s]%d ", root->key, root->value);
...
void print_tree(bstree *tree, int p, int s) {  // string s = string("")) {
  if (tree == NULL) return;
